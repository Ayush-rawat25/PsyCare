...
export default function DMChatSection() {
  ...
  useEffect(() => {
    if (token) {
      fetch("https://psycare-backend.onrender.com/api/chat/online", {
        method: "POST",
    ...
    return () => {
      if (token) {
        fetch("https://psycare-backend.onrender.com/api/chat/offline", {
          method: "POST",
          headers: { Authorization: `Bearer ${token}` },
        });
      }
  ...
  useEffect(() => {
    ...
    const fetchOnline = async () => {
      const res = await fetch("https://psycare-backend.onrender.com/api/chat/online", {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  useEffect(() => {
    ...
    const fetchMessages = async () => {
      const res = await fetch(`https://psycare-backend.onrender.com/api/chat/dm/${selectedUser._id}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      const data = await res.json();
      setMessages(data);
  ...
  const handleSend = async () => {
    ...
    const res = await fetch("https://psycare-backend.onrender.com/api/chat/dm", {
      method: "POST",
      headers: {
